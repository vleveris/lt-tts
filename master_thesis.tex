...
A key limitation of concatenative methods for morphologically rich languages such as Lithuanian is coverage. The large number of distinct inflected word forms increases the probability that certain phonetic contexts are absent from even a large corpus, forcing the system to use poorly matching units or acoustically disruptive concatenations~\cite{chalamandaris2010unit}.
...
The principal advantages of HMM-based synthesis are a small model footprint, the ability to adapt to new speakers with limited data using techniques such as maximum likelihood linear regression (MLLR), and straightforward control over speaking style through model interpolation. The main drawback is that the maximum likelihood estimation inherently averages over the training data, producing over-smoothed parameter trajectories that result in muffled, lacking spectral detail speech. Additional artefacts arise from the vocoder-based waveform reconstruction, which introduces a characteristic buzziness. These shortcomings were the primary motivation for the shift toward neural network-based approaches~\cite{hasanabadi2023overview}.
